...
class Driver:
    ...
    def __str__(self) -> str:
        ...
        >>> d = Driver("Driver1", Location(12, 3), 32)
        >>> print(d)
        """

        return (f"Driver's identifier: {self.id} Driver's location: "
                f"{self.location} Driver's speed: {self.speed}km/h "
                f"Driver is not moving: {self.is_idle}")
