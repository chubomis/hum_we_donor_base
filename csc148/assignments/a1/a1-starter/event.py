...
class RiderRequest(Event):
    ...
    def do(self, dispatcher: Dispatcher, monitor: Monitor) -> List[Event]:
        ...
        if driver is not None:
            duration = driver.start_drive(self.rider.origin)
            events.append(Pickup(self.timestamp + duration,
                                 self.rider, driver))
...
class DriverRequest(Event):
    ...
    def do(self, dispatcher: Dispatcher, monitor: Monitor) -> List[Event]:
        ...
        if rider is not None:
            duration = self.driver.get_travel_time(rider.origin)
...
class Cancellation(Event):
    """
    Represents a Cancellation event initiated by a rider.

    === Attributes ===
    rider: The Rider who initiated the cancellation.
    """
    rider: Rider
    ...
    def do(self, dispatcher: Dispatcher, monitor: Monitor) -> List[Event]:
        ...
        if self.rider.status == WAITING:
            self.rider.change_to_cancelled()
    ...
    def __str__(self) -> str:
        ...
        """
        return f"{self.timestamp} -- {self.rider.id}: Cancel a Ride"
...
class Pickup(Event):
    ...
    driver: The driver picking up the rider.
    """
    ...
    def _pickup_notify(self, monitor: Monitor) -> None:
        monitor.notify(self.timestamp, DRIVER, PICKUP, self.driver.id,
                       self.driver.location)

    def _handle_waiting_rider(self, monitor: Monitor, events: List[Event]) -> \
            None:
        self.rider.change_to_satisfied()

        duration = self.driver.start_ride(self.rider)
        monitor.notify(self.timestamp, RIDER, PICKUP, self.rider.id,
                       self.rider.origin)
        events.append(Dropoff(self.timestamp + duration,
    ...
    def __str__(self) -> str:
        ...
        """
        return f"{self.timestamp} -- {self.driver}: Pick up"


class Dropoff(Event):
    """
    Represents a Dropoff event when a driver drops off a rider.

    === Attributes ===
    driver: The Driver who performs the dropoff.
    ...
    def _dropoff_notify(self, monitor: Monitor) -> None:
    ...
    def do(self, dispatcher: Dispatcher, monitor: Monitor) -> List[Event]:
        ...
        # End the ride for the driver
        self.driver.end_ride()

        # Notify the monitor about the dropoff for both driver and rider
        self._dropoff_notify(monitor)

        # Add a new DriverRequest event
        events.append(DriverRequest(self.timestamp, self.driver))
