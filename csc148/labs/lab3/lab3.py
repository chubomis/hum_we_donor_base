...
class RandomPlayer(Player):
    ...
    def move(self, current, min_step, max_step, goal) -> int:
        if goal - current < max_step:
            result = random.randint(min_step, goal - current)
        else:
            result = random.randint(min_step, max_step)
        return result
...
class UserPlayer(Player):
    ...
    def move(self, current, min_step, max_step, goal) -> int:
        result = int(input("Choose a number between " + str(min_step) + " and " +
                           str(max_step) + ":"))
        return result
...
def make_player(generic_name: str) -> Player:
    ...
    p_type = str(input("Type either UserPlayer or RandomPlayer for your player"
                       " type: "))

    if p_type == "UserPlayer":
