...
class Tree:
    ...
    def __contains__(self, item: Any) -> bool:
        ...
        # item may be in root, or subtrees
        if self._root == item:
            return True
        else:
            for subtree in self._subtrees:
    ...
    def __str__(self) -> str:
        ...
        >>> t1 = Tree(12, [Tree(2, [Tree(1, []), Tree(4, [Tree(12, [])])])])
        >>> print(t1)
        """
    ...
    def helper_branch(self) -> int:
        ...
        if self.is_empty():
            ...
            internal += 1
            for subtree in self._subtrees:
                internal += subtree.helper_branch()
            return internal
    ...
    def branching_factor(self) -> float:
        ...
        if self.is_empty():
            ...
            total_children = 0.0
            for subtree in self._subtrees:
                total_children += subtree.branching_factor()
            return float(math.ceil(total_children / self.helper_branch()))

    def items_at_depth(self, d: int) -> list:
        """Return a list of the values in this tree at the given depth.

        Precondition: d >= 0. (Depth 0 is the root of the tree.)
        ...
        >>> t5.items_at_depth(1)
        [6, 7, 8, 9, 10]
        >>> t6 = Tree(3, [Tree(6, []), Tree(7, [Tree(3, [])]), Tree(9, [Tree(10, []), Tree(8, [])])])
        >>> t6.items_at_depth(2)
        [3, 10, 8]
        """
        if self.is_empty():
